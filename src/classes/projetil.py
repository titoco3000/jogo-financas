...
class Projetil(GameObject):
    def __init__(self, x, y, angle, ativo=True):
        ...
        if self.ativo:
            if globals.efeitos_no_jogador.has(efeitos.ZigZagProjetil):
                ...
                if self.steps_in_direction == 1:
                    self.alt_direction *= -1
                    self.dx = (
                        math.cos(self.angle + 0.5 * self.alt_direction) * bullet_speed
                    )
                    self.dy = (
                        math.sin(self.angle + 0.5 * self.alt_direction) * bullet_speed
                    )
            ...
            if (
                ...
                or self.x > globals.screen_size.x + bullet_radius
            ):
                self.__del__()
            else:
                ...
                for enemy in inimigos:
                    ...
                    closest_x = max(
                        enemy_rect.left, min(bullet_center.x, enemy_rect.right)
                    )
                    closest_y = max(
                        enemy_rect.top, min(bullet_center.y, enemy_rect.bottom)
                    )
